...
- **Rationale helper:** Add `src/lib/planRationale.ts` with a pure function `buildRationale(args: { sqftTier: string | null; yardTier: string | null; familyName: string; variantName: string }): string`. Named-params object so the yard-fallback branch is cleanly added. Reverse-maps `home_sqft_tier` to a readable range, produces `"Based on your ~2,000 sqft home, your Basic plan is Basic 20."` or (when tier is null) `"Based on your property profile, your Basic plan is Basic 20."`. If sqft is missing but `yardTier` is present, uses the yard-based sentence. `windows_tier` / `stories_tier` aren't surfaced — all seed rules are sqft-primary, and yard is enough for a fallback.
  - Ranges:
