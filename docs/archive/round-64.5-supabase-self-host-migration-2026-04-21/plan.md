...
| C-4 | Re-add secrets to new project | Claude | ✅ | All 6 secrets pushed via `supabase secrets set`: `ANTHROPIC_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `RESEND_API_KEY`, `CRON_SECRET`, `WEATHER_API_KEY`. Digests verified via `supabase secrets list`. End-to-end smoke via cron helper got past the WEATHER_API_KEY check (the prior 400 became a 500 with `[object Object]` from a pre-existing `check-weather` error-serialization bug — flagged in TODO.md as a separate cleanup). |
...
| C-6 | pg_cron repoint to Vault | Claude | ✅ | Discovered the dashboard SQL editor also lacks `ALTER DATABASE` privilege (Supabase Cloud locked the postgres role). Pivoted to Vault: pushed `service_role_key` into `vault.secrets` via Management API, wrote migration `20260421050000_round_64_5_repoint_cron_jobs_to_vault.sql` (creates `cron_private.invoke_edge_function` SECURITY DEFINER helper + reschedules all 7 jobs through it). Applied via Management API, recorded in `schema_migrations`. End-to-end smoke test: helper → check-weather returned 400 "WEATHER_API_KEY not configured" — confirms vault decrypt + service_role auth + net.http_post path all work. URL hardcoded (not secret); key never enters git. |
...
| D-2 | Stripe webhook URL update | User | ✅ | New webhook destination created on Stripe pointing at `https://gwbwnetatpgnqgarkvht.supabase.co/functions/v1/stripe-webhook` with the 9 modern events (after Stripe retired transfer.paid/failed in the 2025 API). Signing secret pushed via Phase C-4. OLD webhook on Lovable project still active — user to disable it after main merge. |
...
| E | Smoke test against new project | User | ✅ | User signed up as customer at `https://handledhome.app/auth` — landed cleanly on customer onboarding pages. Confirms: Vercel env vars correctly target new project, Supabase Auth on new project works, AuthContext + ProtectedRoute + CustomerOnboardingWizard chain all intact. (Tested email+password; the app does not currently expose Google OAuth in the UI — the Google provider config is pre-staged for a future button.) |
| F | Commit `.env` flip + user updates Stripe/Google | Both | 🟡 | `.env` flipped + committed on feature branch. Awaiting user decision on feature→main merge before Phase G archive. After merge, user disables OLD Stripe webhook on legacy Lovable project. |
...
- **Branch:** `claude/supabase-self-host-migration`. Pushed tip: `e14c44c` (stripe-webhook transfer event rename + C-4 partial secrets push).
...
- **Next up:**
  - **User actions (remaining):**
    1. **`WEATHER_API_KEY`** from weatherapi.com — last C-4 secret. Non-blocking for Phase E overall; only `check-weather` is affected.
    2. **`LOVABLE_DIRECT_DB_URL`** for Phase C-2 data migration (or say "use CSV fallback").
...
- **Round progress:** 12 / 16 phase-steps complete (B-1, B-2, C-1, C-3, **C-4**, C-5, C-6, C-7, D-1, D-3) + skipped C-2 (~75%). Remaining: dashboard Auth config (user), Phase E smoke test (user + Claude), Phase F cutover commit (user signal → Claude), Phase G Round 64 resume.
...
- `[OVERRIDE: C-6 approach changed — original plan called for ALTER DATABASE postgres SET app.settings.{supabase_url,service_role_key}. On the new Supabase Cloud project, the postgres role lacks ALTER DATABASE privilege from BOTH the Management API and the dashboard SQL editor (verified: ERROR 42501). Pivoted to a Vault-based approach: service_role_key stored in vault.secrets, accessed via SECURITY DEFINER helper cron_private.invoke_edge_function. URL hardcoded in helper (not a secret). Migration 20260421050000 records the change. Same end-state — 7 cron jobs invoke Edge Functions with service_role_key — but via a path that doesn't need privileged ALTER DATABASE. 2026-04-21.]`
