...
5. The `e2e` job validates 7 required secrets (test-user credentials + bypass secret) and fails fast with a clear `::error` annotation when any are missing. `TEST_BYOC_TOKEN` is optional — its absence is handled by `test.skip` guards in `byoc-happy-path.spec.ts` and `byoc-refresh-resilience.spec.ts`, so BYOC specs skip gracefully rather than crashing mid-run.
