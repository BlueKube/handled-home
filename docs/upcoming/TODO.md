...
### 2026-04-24 — Anthropic API credit balance — reported top-up did NOT propagate; needs user verification (T.6/T.7 follow-up)

> **Status update (2026-04-24, post-T.7 merge):** The user reported adding credits and asked me to continue. PR #26's CI (run `24866330199`, commit `25a7e29`) ran the ai-judge customer shard after credits were reportedly added, and the API returned the **same** 400 credit-balance-too-low error as pre-topup (request_id `req_011CaMgALo1eiEVKaxxoDmJs`). T.6's 🛑 error banner surfaced this cleanly in the PR comment — proof the error-surface path works — but the underlying billing state is unchanged. Reopening this item pending user verification of which account/key actually received the credits.
>
> **Likely causes (order):** (1) credits added to a different Anthropic workspace than the one that issued `ANTHROPIC_API_KEY` in GH Secrets; (2) the API key currently in GH Secrets was generated from a different workspace; (3) billing propagation delay (unlikely given hours elapsed); (4) monthly usage cap separate from credit balance.
>
> **Verification steps for user:** (a) Anthropic Console → Settings → API Keys — identify which workspace the key in GH Secrets belongs to (match the `sk-ant-api03-...` prefix). (b) In that specific workspace → Plans & Billing, confirm non-zero credit balance. (c) If credits were added to a different workspace, either move them or generate a new key on the funded workspace and update `ANTHROPIC_API_KEY` in GitHub Settings → Secrets and variables → Actions → Repository secrets (`handled-home` repo). (d) After the secret is updated, re-run any subsequent PR's CI — the 🛑 banner should flip to real Sarah scores in the Customer row automatically.

- [ ] **Refill credits on the Anthropic account backing `ANTHROPIC_API_KEY` in GH Actions Secrets, or rotate to a funded key.** PR #25's CI (commit `0eba41f`) proved the T.6 model-ID fix landed correctly — the new ID `claude-haiku-4-5-20251001` is valid — but every Anthropic call now returns HTTP 400 with `{"type":"invalid_request_error","message":"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."}`. The error surfaces cleanly in the PR comment via the new 🛑 advisory banner T.6 added (Advisory section: `Customer AI evaluation crashed against model claude-haiku-4-5-20251001: 400 ...`). No code action available — this is a console visit to https://console.anthropic.com/settings/billing.
  - **Why:** Until credits are refilled, every PR's Tier 5 matrix costs $0 (scripts crash before any billable token) but produces zero scoring signal. The T.3 visibility layer + T.6 error surface guarantee the issue is visible; it's just not auto-fixable.
  - **Blocked:** Live Sarah-persona signal on every PR. T.6's "real scores appear" acceptance criterion is formally blocked by this, though the T.6 fix itself is verified working.
  - **What to do:** (1) Go to Anthropic Console → Plans & Billing → Add credits or upgrade. (2) If preferring to switch accounts: generate a new API key on the funded account, update `ANTHROPIC_API_KEY` in GitHub Settings → Secrets and variables → Actions → Repository secrets, revoke the old key. (3) Re-run PR #25's CI (push empty commit or Re-run job) to confirm real scores appear. Alternatively, remove the secret entirely to kill Tier 5 spend temporarily — the workflow is designed to skip AI judge gracefully when the key is unset.
  - **Source:** PR #25 CI run 24859813878 (commit 0eba41f) — 🛑 banner explicitly surfaced the billing error, which was the intended behavior after T.6.
