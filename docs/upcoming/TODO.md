...
- [ ] **`STRIPE_WEBHOOK_SECRET`** — user creating webhook destination in Stripe dashboard. URL: `https://gwbwnetatpgnqgarkvht.supabase.co/functions/v1/stripe-webhook`. Events: 9 types (see stripe-webhook/index.ts). **Note 2026-04-21 session 2:** `transfer.paid` + `transfer.failed` may not appear in the event picker until a Stripe Connect account is onboarded. If absent, subscribe to the other 7 events for now and add a follow-up item below.
- [ ] **Stripe webhook — enable `transfer.paid` + `transfer.failed` before first real provider payout.** These events drive `provider_payouts.status` + `provider_earnings.status` transitions in `stripe-webhook` (cases `transfer.paid` and `transfer.failed`). Without them, payouts sit in `PROCESSING` forever and earnings stay in `IN_PAYOUT`. Options when the time comes: (a) expose them by onboarding a Connect test account first, then edit the destination to add the two events; (b) create a second "Connected and v2 accounts" destination pointing at the same URL — requires adding a second webhook secret check to `stripe-webhook/index.ts` since Stripe signs each destination with its own secret.
