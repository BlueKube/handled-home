    ...
    <div className="px-4 py-6 space-y-4 animate-fade-in pb-24 max-w-lg mx-auto">
      ...
      {invoices.length === 0 ? (
        <CustomerEmptyState
          ...
          body="Invoices and receipts will appear here after your first billing cycle."
        />
      ) : (
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={(statusColors[inv.status] ?? "secondary") as any} className="text-[10px]">
                      {inv.status}
                    </Badge>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {inv.cycle_start_at && inv.cycle_end_at
                      ? `${new Date(inv.cycle_start_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} – ${new Date(inv.cycle_end_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`
