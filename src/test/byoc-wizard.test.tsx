...
vi.mock("@/hooks/useByocOnboardingContext", () => ({
  useByocOnboardingContext: () => ({
    ...
    activate: { mutateAsync: vi.fn().mockResolvedValue({}) },
...
// Mock image imports
vi.mock("@/assets/handled-home-logo.png", () => ({ default: "test-logo.png" }));
...
describe("ByocOnboardingWizard", () => {
  describe("Invalid invite → fallback", () => {
    it("shows fallback when invite is null", () => {
      mockInviteData = null;
      render(<ByocOnboardingWizard />, { wrapper: Wrapper });
      expect(screen.getByText("This invitation is no longer active")).toBeInTheDocument();
    });

    it("shows fallback when is_active=false", () => {
      mockInviteData = { id: "inv-1", provider_org: { id: "org-1", name: "Co", logo_url: null }, sku: null, category_key: "mowing", zone_id: "z1", is_active: false, default_cadence: "weekly" };
      render(<ByocOnboardingWizard />, { wrapper: Wrapper });
      expect(screen.getByText("This invitation is no longer active")).toBeInTheDocument();
    });

    it("fallback buttons present", () => {
      mockInviteData = null;
      render(<ByocOnboardingWizard />, { wrapper: Wrapper });
  ...
  describe("Valid invite → recognition", () => {
    beforeEach(() => {
    ...
    it("'I'm new here' navigates to onboarding", () => {
      render(<ByocOnboardingWizard />, { wrapper: Wrapper });
...
describe("Back nav skips activating", () => {
  const STEPS = ["recognition", "confirm", "property", "home_setup", "activating", "services", "plan", "success"] as const;
  function handleBack(s: string) {
    const i = STEPS.indexOf(s as any);
    let p = i - 1;
    if (STEPS[p] === "activating") p--;
    return p >= 0 ? STEPS[p] : s;
  }
