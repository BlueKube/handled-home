...
BEGIN
  -- 1. Fetch visit with schedule_state filter
  ...
  -- Extract as text first, then cast to int (handles both JSON number and string)
  BEGIN
  ...
  IF p_action_type = 'running_late' THEN
    ...
    ELSE
      ...
      -- Try to compute new ETA and check window breach
      BEGIN
