...
-- P0 Fix: propose_provider_action — correct emit_notification_event call
-- Uses p_priority + p_payload instead of non-existent p_title/p_body/p_data

DROP FUNCTION IF EXISTS public.propose_provider_action(uuid, text, jsonb);

CREATE FUNCTION public.propose_provider_action(
  p_visit_id uuid,
  p_action_type text,
...
BEGIN
  ...
  IF p_action_type = 'running_late' THEN
    ...
    ELSE
      ...
      -- Notify customer via event bus (correct 9-param signature)
        ...
        BEGIN
          PERFORM emit_notification_event(
            ...
            p_payload := jsonb_build_object(
              ...
              'body', format('New ETA: %s minutes later than originally scheduled', v_eta_slip_minutes),
              'visit_id', p_visit_id,
              'slip_minutes', v_eta_slip_minutes
            )
          );
