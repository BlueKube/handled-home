...
-- Sprint 3D Phase B3: Add predicted suggestions to get_service_suggestions RPC
...
BEGIN
  ...
  -- 8. AI predictions (non-expired)
  predictions AS (
    SELECT sku_id, confidence, reason AS pred_reason, timing_hint
  ...
  candidates AS (
    SELECT
      ...
      + CASE
          ...
          ELSE 0
        END
      -- AI prediction boost (confidence / 10)
      + COALESCE(pred.confidence / 10, 0)
      AS score,
      -- Suggestion type: predictions override
      CASE
        WHEN pred.sku_id IS NOT NULL THEN 'predicted'
      ...
      -- Reason string: predictions use AI reason
      CASE
        WHEN pred.sku_id IS NOT NULL THEN pred.pred_reason
    ...
    LEFT JOIN predictions pred ON pred.sku_id = s.id
    WHERE s.status = 'active'
      AND s.is_addon = false
      AND (
        (SELECT has_data FROM has_coverage) = false
        OR s.category IN (SELECT category_key FROM eligible_cats)
        OR pred.sku_id IS NOT NULL  -- Always include predicted SKUs even if category not in eligible
      )
