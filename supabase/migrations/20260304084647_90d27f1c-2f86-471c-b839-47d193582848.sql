...
-- Sprint 8 Phase 3: Reactive Exception RPCs

-- ── 1) Provider report issue RPC ──
...
BEGIN
  ...
  IF p_issue_type = 'access_failure' THEN
    ...
    INSERT INTO customer_reschedule_holds (
      ...
      CASE WHEN extract(dow from v_visit.scheduled_date + 1) IN (0, 6)
           THEN v_visit.scheduled_date + 3 ELSE v_visit.scheduled_date + 1 END,
  ...
  PERFORM emit_notification_event(
    ...
    p_priority := CASE WHEN v_severity = 'urgent' THEN 'HIGH' ELSE 'NORMAL' END,
...
-- ── 2) Customer request reschedule RPC ──
...
BEGIN
  ...
  IF v_visit.schedule_state IN ('completed', 'canceled', 'exception_pending') THEN
  ...
  PERFORM emit_notification_event(
    p_event_type := 'ADMIN_EXCEPTION_CREATED',
    p_idempotency_key := 'admin_exception:' || v_exception_id,
    p_audience_type := 'ADMIN', p_priority := 'NORMAL',
...
-- ── 3) Confirm reschedule hold RPC ──
CREATE OR REPLACE FUNCTION public.confirm_reschedule_hold(
  p_hold_id uuid,
  p_action text  -- 'confirm' or 'release'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_action = 'confirm' THEN
    ...
    PERFORM emit_notification_event(
      ...
      p_audience_type := 'USER', p_audience_user_id := v_hold.customer_id,
      p_priority := 'NORMAL',
...
-- ── 4) Apply customer reschedule (chosen date) ──
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Visit not found'; END IF;
  IF v_visit.customer_id != v_caller_id THEN RAISE EXCEPTION 'Not authorized'; END IF;
  IF v_visit.schedule_state IN ('completed', 'canceled') THEN
  ...
  PERFORM emit_notification_event(
    ...
    p_audience_type := 'USER', p_audience_user_id := v_visit.customer_id,
    p_priority := 'NORMAL',
