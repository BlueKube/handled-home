...
-- Review Fix: propose_provider_action — migrate emit_notification() to emit_notification_event()
-- Review Fix: job_assignment_log RLS — revert scope escalation (ALL → SELECT)
-- Review Fix: Add consistent TO authenticated on all policies

-- 1. Fix propose_provider_action notification call
...
BEGIN
  ...
  IF p_action_type = 'running_late' THEN
    ...
    ELSE
      ...
      -- Notify customer via event bus (replaces legacy emit_notification)
      ...
      IF v_customer_id IS NOT NULL THEN
        BEGIN
          PERFORM emit_notification_event(
            p_event_type := 'CUSTOMER_ETA_UPDATED',
            p_idempotency_key := 'eta_slip:' || p_visit_id::text || ':' || now()::text,
            p_audience_type := 'CUSTOMER',
            p_audience_user_id := v_customer_id,
            p_audience_zone_id := v_visit.zone_id,
            p_title := 'Your provider is running late',
            p_body := format('New ETA: %s minutes later than originally scheduled', v_eta_slip_minutes),
            p_data := jsonb_build_object('visit_id', p_visit_id, 'slip_minutes', v_eta_slip_minutes)
          );
...
-- 2. Fix job_assignment_log RLS: revert from ALL back to SELECT-only + add TO authenticated
...
CREATE POLICY "Admins read job_assignment_log"
  ON public.job_assignment_log FOR SELECT TO authenticated
  USING (public.is_admin_member(auth.uid()));

-- Keep the separate INSERT policy (already exists from prior migration)
...
CREATE POLICY "Admin inserts job_assignment_log"
  ON public.job_assignment_log FOR INSERT TO authenticated
  WITH CHECK (public.is_admin_member(auth.uid()));

-- 3. Fix notification_events: add missing TO authenticated
